...
const mutations =  {
      ...
      ['TOGGLE_SHUFFLE'](state) {
        ...
        if (state.shuffle) {
          ...
          state.position = tracksHelper.getKeyByPositionForTrack(state.tracks, state.position)
        } else {
        ...
        if (getters.isFirst) {
          if (!state.repeat) return;
            if (state.shuffle) {
              ...
              tracksHelper.trackMoveLastFirst(tracks)
      ...
      next({commit, state, getters }) {
        if (getters.isLast) {
          if (!state.repeat) return;
            if (state.shuffle) {
              let tracks = state.tracks
              tracksHelper.trackMoveFirstLast(tracks)
              commit('SET_TRACKS', tracks)
              const track = state.tracks[state.position]
              commit('SET_TRACK', track)
